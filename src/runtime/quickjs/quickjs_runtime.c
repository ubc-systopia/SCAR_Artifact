...
void quickjs_get_bytecode_handler_cacheline() {
	...
	log_info("Target base address: %lx, target goto8 address: %lx, target sar "
	         "address: %lx",
	         target_base,
	         target_goto8,
	         target_sar);
...
void quickjs_eval_buf_loop(JSRuntime *rt,
	...
	do {
		...
		char *sync_data_str = (char *)sync_ctx.data;
		if (strlen(sync_data_str) > 0) {
			putenv(sync_data_str);
		}
...
void *quickjs_runtime_thread(void *param) {
	quickjs_runtime_thread_config_t *pt_config =
	    (quickjs_runtime_thread_config_t *)param;
	if (pt_config->pin_cpu != -1) {
		/* iso_pin_cpu(pt_config->pin_cpu); */
	}
	char js_eval_file[256];
	...
	if (pt_config->js_eval_file[0] == '/') {
		// if absolute path
		snprintf(
		    js_eval_file, sizeof(js_eval_file), "%s", pt_config->js_eval_file);
	} else {
		// if relative path
		snprintf(js_eval_file,
		         sizeof(js_eval_file),
		         "%s/%s",
		         cfg->project_root,
		         pt_config->js_eval_file);
	}

	quickjs_init(&rt, &ctx);

	log_info("QuickJS runtime thread warmup: %s", js_eval_file);
	// warm up
	int warm_up_iter = 2;
	for (int i = 0; i < warm_up_iter; ++i) {
		js_std_eval_file(ctx, js_eval_file, -1);
	}
	...
	for (int i = 0; i < victim_runs; ++i) {
		pthread_barrier_wait(sync_ctx.barrier);
		log_info("QuickJS runtime thread iteration: %d", i);
	...
	tsc1 = rdtscp();
	log_debug("Quickjs runtime execution rdtsc:\n"
	          "pid:\t%d\n"
	          "start:\t%ld\n"
	          "end:\t%ld\n"
	          "per run:\t%lf",
