...
Two panels. On top, a single real loop iteration per value of the secret bit — one lane
each, not a distribution. Every marker is a detected access, coloured by bit (same hue as
the shaded span and the violins below) and shaped by which libbf call it is — filled for
`bf_rint`, hollow for `bf_logic_or` — in the order the loop makes them. Each lane's
iteration is picked as the one whose `SELECT` gap is closest to that bit's median across
the whole trace, so it is a typical instance, not a cherry-picked one. The shaded span and
the bracket both mark what the attacker measures, `rint(r · x mod n)` to `or`, which is
all of `SELECT`; the bracket label rounds to the nearest 1,000 cycles (the exact figure is
in the violin panel below).

Below it, the distribution those two instances were drawn from: every `SELECT` gap in the
trace, split by the true bit, as violins with a thin Tukey box (median + IQR) inside each —
the same subfigure style, stats box, and Δmedian/AUC readout as `openpgp_patch/plot_dist_paper.py`'s
figure 4b. It is what "typical" in the top panel means concretely: both picked instances
sit at their violin's median.

![One real loop iteration on the watched line, bracketed on the SELECT interval, for bit 0 and bit 1, over the full per-bit gap distribution](results/figures/fig6_gap.svg)

The two modular multiplications either side of `SELECT` are ~278,000 cycles each — nine
times the interval that carries the bit — so they are drawn compressed and marked as
such; everything between the bracketed endpoints is at true scale.

Three things are worth reading off it. The lane shows why the attack has anything to
measure at all: `bf_rint` puts a marker on both sides of `SELECT`, on the same cache line
as the `|` that closes it. The two brackets show the shift is real and consistent between
one iteration and another — this pair of instances happens to land almost exactly on
their bits' respective medians, and the violins below show the two full distributions
barely overlap (AUC 0.997). And they show it is *small*: 2,316 cycles inside a
~30,000-cycle interval that is itself only 5% of the 611,000-cycle iteration. The attacker
is not looking for a conspicuous pause, only for a consistent one.

(Generated by `analysis/plot_gap_figure.py` from `data/traces/r0.out`, with plotly, same
palette/font/template as `openpgp_patch/plot_dist_paper.py`; 2,032 iterations with a 0 bit
and 1,959 with a 1 in that trace, of which one representative pair is shown.)
...
What is used instead is a single global model, `timestamp = bit_index × period + t₀`. The
period is the median spacing between consecutive pairs: a missed iteration makes some
spacings 2P or 3P, which move the mean but not the median. That is the whole estimator,
one line (`analysis/decoder.py::loop_period`).

**Everything then depends on the period being right, and this estimator is not right
enough.** A relative error ε displaces the index by ε × n, so across 4094 iterations
ε ≳ 1.2 × 10⁻⁴ is enough to slip an index before the trace ends — one part in 8000 — and
once it slips it stays slipped. The median spacing is quantised by the probe's own timing
granularity and lands a few parts in 10⁵ off, which is enough. So each trace tracks the
key over a contiguous stretch and then falls to chance for the rest:

```
r0:  0.939 0.638 0.796 0.897 0.958 0.411 0.039 0.113 0.019 -0.022   (correlation per tenth)
r4: -0.000 0.432 0.900 0.950 0.876 0.905 0.896 0.913 0.876  0.862
```

This is the price of the simple estimator, and it is where essentially all the lost
accuracy goes: inside the tracking stretch the bit rule is as good as it ever was
(r0's lowest 2048 bits: 23 errors in 2013), and outside it there is no signal at all
because the bits are being compared against the wrong positions. A decoder that pins the
period to ~10⁻⁸ relative — by scanning candidate periods for the one whose phases `t / P`
cluster most tightly onto integers, rather than reading a median off the spacings —
holds phase for the whole trace and reaches 0.97–0.99. That is a real attack improvement
and a much harder thing to explain; this artifact deliberately keeps the explainable
version. See "Honest limits" below.
...
so it is one unknown for the whole trace. An attacker carries the candidates forward; the
evaluation here resolves it against the known key (`decoder.best_anchor`).
...
|-------|-----------:|------------:|---------------------:|--------------:|---------:|----------------------:|
| r0    | 4023 | 0.982 | +0.4819 | 4022 | 0.7718 | −0.0186 (0.4915) |
| r1    | 3988 | 0.979 | +0.4724 | 3987 | 0.7750 | −0.0034 (0.4974) |
| r2    | 3987 | 0.975 | +0.6170 | 3986 | 0.8989 | −0.0158 (0.4910) |
| r3    | 4008 | 0.977 | +0.4817 | 4006 | 0.7646 | −0.0133 (0.4938) |
| r4    | 4024 | 0.979 | **+0.7628** | 4022 | **0.9177** | −0.0132 (0.4955) |

A correlation of 0.47–0.76 is 30 to 49 times the noise floor (1/√4094 = 0.0156), and the
forward-order control stays at chance in every trace.

Accuracy is *not* flat along the trace — splitting each into ten consecutive parts:

```
r0:  0.939 0.638 0.796 0.897 0.958 0.411 0.039 0.113 0.019 -0.022
r3:  0.747 0.873 0.937 0.865 0.837 0.399 0.033 0.084 -0.002 -0.020
r4: -0.000 0.432 0.900 0.950 0.876 0.905 0.896 0.913 0.876  0.862
```

![Correlation per tenth of the trace: high over a contiguous stretch, chance elsewhere](results/figures/fig5_along_trace.png)

Every trace has a stretch where it tracks the key at 0.9 or better and a stretch where it
is at zero, and the boundary moves from trace to trace. This is the period-error slip
described above, not a property of the leak: inside the tracking stretch the bit rule
works exactly as well as the gap distributions say it should.
...
|---------------|----:|-----:|-----:|---------:|
| r0 | 0.979 | 0.987 | 0.989 (2013 assigned) | 0.772 (4022) |
| r1 | 0.510 | 0.528 | 0.555 (1986) | 0.775 (3987) |
| r2 | 0.514 | 0.743 | 0.868 (2008) | 0.899 (3986) |
| r3 | 0.881 | 0.961 | 0.975 (2007) | 0.765 (4006) |
| r4 | 0.516 | 0.693 | 0.840 (2007) | 0.918 (4022) |

**Trace r4 recovers 4022 of the 4094 bits of the private exponent with 331 errors, from a
single signature**, and trace r0 recovers the lowest 2048 bits with 23.

Whether that is enough to finish the key depends on which trace and which bit range. The
partial-key-exposure results recover a full RSA key from the lowest quarter of the private
exponent with a small public exponent; r0 and r3 clear that comfortably (0.99 and 0.96
over the lowest 1024 bits), while r1, r2 and r4 do not, because their tracking stretch
starts higher up. This decoder therefore does not give a uniform "attack succeeds" claim
the way its predecessor did — it gives a per-trace one, and the lattice step is in any case
cited from the literature rather than run here.

**Taking more traces does not improve accuracy**, because the remaining errors are largely
shared across traces rather than independent — traces lose phase in the same places:

| traces combined | 1 | 2 | 3 | 4 | 5 |
|---|---:|---:|---:|---:|---:|
| accuracy | 0.9180 | 0.9134 | 0.9132 | 0.9086 | 0.8828 |
| bits covered | 4000 | 4087 | 4092 | 4094 | 4094 |

Here it makes things actively worse: the best single trace beats every combination, and
each trace folded in drags the vote toward its own dead region. What combination buys is
coverage — one trace leaves ~90 bit positions with no pair assigned, and three cover
essentially all 4094. Comparing r0 and r3 directly makes the shared-error point plainly:
over bits 2500–3500 both sit at chance (0.51–0.53) while agreeing with *each other* 98.5%
of the time.

## 8. Honest limits

- One key, five signatures. Nothing here shows the numbers hold across keys or machines.
- **The decoder is deliberately simpler than the attack allows.** Estimating the period as
  a median spacing costs 15–20 accuracy points against locking it to ~10⁻⁸ relative, and
  costs them unevenly: each trace goes from ~0.95 to chance at whatever point the
  accumulated phase error slips an index. The numbers above are therefore a lower bound on
  what this side channel yields, chosen so the whole decoder can be stated in four lines.
...
per loop iteration. Reading the loop period off the median spacing between those pairs —
the simplest estimator there is — one signature yields 4022 of the 4094 bits of the private
exponent at 0.92 accuracy, or the lowest 2048 bits with 23 errors on the best-placed trace.
A period estimated tightly enough to hold phase across all 4094 iterations pushes that to
0.97–0.99, but needs a good deal more machinery than the four lines this decoder is built
from.
