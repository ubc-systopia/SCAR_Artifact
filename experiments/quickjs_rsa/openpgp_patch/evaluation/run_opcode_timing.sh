...
SOURCE_DIR="$(cd "$PATCH_DIR/../../../third_party/quickjs" && pwd)"
BUILD_DIR="$(mktemp -d /tmp/selectBigInt-qjs.XXXXXX)"
OUTPUT_PREFIX="${OUTPUT_PREFIX:-$PATCH_DIR/opcode_timing}"
...
for ((run = 1; run <= RUNS; run++)); do
	...
	QJS_OPCODE_TIMING_OUT="$output" \
		...
		"$PATCH_DIR/js/selectBigInt_opcode_harness.mjs" "$SAMPLES" "$BITS"
done
