...
echo "[*] sel func    :" $(for p in "$SCRIPT_DIR"/js/*.mjs; do basename "$p" .mjs; done)
...
# ---- collect -------------------------------------------------------------
for p in "$SCRIPT_DIR"/js/*.mjs; do
	impl="$(basename "$p" .mjs)"
	impl_module="./js/${impl}.mjs"      # resolved relative to bench.mjs
