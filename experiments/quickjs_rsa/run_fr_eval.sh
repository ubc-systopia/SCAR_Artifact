#!/usr/bin/env bash
# End-to-end Flush+Reload run: victim + quickjs_rsa_fr attacker, then decode.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
BUILD="${ROOT}/build"
CPUS="${CPUS:-1,3,5,7,9,11,13,15}"
VICTIM_RUNS="${VICTIM_RUNS:-1}"
WAITING_TIME="${WAITING_TIME:-20000}"
...
for f in "$VICTIM_BIN" "$ATTACKER_BIN"; do
	if [ ! -x "$f" ]; then
		echo "Missing binary: $f (build quickjs_rt and quickjs_rsa_fr first)" >&2
		exit 1
	fi
done
...
echo "== Starting FR attacker (VICTIM_RUNS=${VICTIM_RUNS}, WAITING_TIME=${WAITING_TIME}) =="
VICTIM_RUNS="$VICTIM_RUNS" WAITING_TIME="$WAITING_TIME" \
...
TRACE_DIR="${BUILD}/output/quickjs_openpgp_rsa_fr_r$(printf '%05d' "$VICTIM_RUNS")"
echo "== Trace directory: ${TRACE_DIR} =="
ls -la "$TRACE_DIR" || { echo "No trace directory produced" >&2; exit 1; }

TRACE_FILE="${TRACE_DIR}/r0.out"
echo "== Decoding ${TRACE_FILE} =="
cd "${ROOT}/experiments/quickjs_rsa/evaluation"
PYTHON="${ROOT}/.venv/bin/python3"
[ -x "$PYTHON" ] || PYTHON=python3
"$PYTHON" "$EXTRACT" -f "$TRACE_FILE" --at FR --id 0 --no-cache \
	--sample-interval "$WAITING_TIME"
